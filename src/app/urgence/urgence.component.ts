...
opendetail(content, type, modalDimension,id) {
   this.urgenceDetails=this.searchiId(id,listInfo)
  if (modalDimension === 'sm' && type === 'modal_mini') {
      this.modalService.open(content, { windowClass: 'modal-mini', size: 'sm', centered: true }).result.then((result) => {
          this.closeResult = `Closed with: ${result}`;
      }, (reason) => {
  ...
  this.imgDetails="./assets/img/theme/h2.jpg";
  this.nameDetails=this.urgenceDetails[0].name
